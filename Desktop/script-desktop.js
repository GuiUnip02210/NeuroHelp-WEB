...
        // tecnicoIdLogado será o ID do usuário logado
        const tecnicoIdLogado = payload && payload[idClaim] ? parseInt(payload[idClaim]) : null;

        const tecnicoActionsBlock = $("#tecnico-actions"); // Referência ao bloco de Alterar Status

       // 2. Definir as regras de exibição
        const isTecnicoAtribuido = chamado?.tecnicoId === tecnicoIdLogado;

        if (tecnicoActionsBlock) {
            if ((tipoUsuario === "2" && isTecnicoAtribuido) || tipoUsuario === "3") {
                // Se for Técnico ATRIBUÍDO (Tipo 2) OU um Administrador (Tipo 3)
                tecnicoActionsBlock.style.display = "block"; // Torna visível
                console.log("Ações do Técnico: Visível.");
            } else {
                // Usuário Comum, ou Técnico não atribuído
                tecnicoActionsBlock.style.display = "none"; // Torna oculto
                console.log("Ações do Técnico: Oculto.");
            }
        }
